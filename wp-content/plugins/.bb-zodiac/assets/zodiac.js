jQuery(function ($) {
    ...
    const Validator = {
        ...
        dob(inputId, errorId) {
            ...
            $error.text('');
            if (!value) {
                $error.text('Vui lòng nhập ngày sinh.');
                $input.addClass('is-error');
                return false;
            }
            const match = value.match(/^(\d{1,2})[/\-\.\s](\d{1,2})[/\-\.\s](1[8-9]\d{2}|20\d{2})$/);
            if (!match) {
                $error.text('Vui lòng nhập năm sinh thực tế (VD: 15/12/1999).');
                $input.addClass('is-error');
                return false;
            }
            ...
            if (month < 1 || month > 12) {
                $error.text('Tháng không hợp lệ.');
                $input.addClass('is-error');
                return false;
            }
            if (day < 1 || day > 31) {
                $error.text('Ngày không hợp lệ.');
                $input.addClass('is-error');
                return false;
            }
            const testDate = new Date(year, month - 1, day);
            if (testDate.getFullYear() !== year || testDate.getMonth() !== month - 1 || testDate.getDate() !== day) {
                $error.text('Ngày ' + day + '/' + month + ' không tồn tại.');
                $input.addClass('is-error');
                return false;
            }
            return value;
        ...
        init() {
            ...
            $form.on('submit', function (e) {
                ...
                $.post(ZodiacAjax.api_url + 'calc', {dob: dob, zdc_cbsp: $('#zdc-cbsp').val() || ''}, function (res) {
                    if (!res.success) {
                        $('#zdc-error-dob').text(res.data.message || 'Đã có lỗi xảy ra.');
                    ...
                    $('#zdc-error-dob').text('Lỗi kết nối. Vui lòng thử lại.');
                    $submitBtn.removeClass('zdc-loading').removeAttr('disabled');
                });
            });
        ...
        init() {
            ...
            $loveForm.on('submit', function (e) {
                ...
                $.post(ZodiacAjax.api_url + 'love', {
                    ...
                    if (!res.success) {
                        $('#zdc-error-love-name1').text(res.data?.message || 'Đã có lỗi xảy ra.');
                    ...
                    $('#zdc-error-love-name1').text('Lỗi kết nối. Vui lòng thử lại.');
                    $submit.removeClass('zdc-loading').removeAttr('disabled');
                });
            });
            ...
            $result.on('click', '#zdc-love-btn-deep-analyze', function () {
                ...
                $.post(ZodiacAjax.api_url + 'love-analyze', {
                    ...
                    if (resAI.success) {
                        ...
                        $('#zdc-love-deep-analysis-form').find('.zdc-err-analyze').text(resAI.data?.message || 'Lỗi không xác định.');
                        $('#zdc-love-tab-chat-ai-content').html('').hide();
                        $('html, body').animate({scrollTop: $('#zdc-love-deep-analysis-form').offset().top - 50}, 400);
                    }
                }).fail(function () {
                    $('#zdc-love-deep-analysis-form').find('.zdc-err-analyze').text('Lỗi kết nối. Vui lòng thử lại.');
                    $('#zdc-love-tab-chat-ai-content').html('').hide();
                    $('html, body').animate({scrollTop: $('#zdc-love-deep-analysis-form').offset().top - 50}, 400);
                }).always(function () {
    ...
    const TuviHandler = {
        init() {
            ...
            $form.on('submit', function (e) {
                ...
                if (!sign || !period) {
                    $error.text('Vui lòng chọn chòm sao và thời điểm để kết nối năng lượng.');
                    return;
                }
                ...
                $.post(ZodiacAjax.api_url + 'tuvi', {
                    ...
                    if (res.success) {
                        ...
                        $btn.removeClass('zdc-loading').removeAttr('disabled');
                        $error.text(res.data.message || 'Bị gián đoạn. Vui lòng thử lại.');
                    }
                }).fail(function () {
                    $btn.removeClass('zdc-loading').removeAttr('disabled');
                    $error.text('Không thể kết nối. Vui lòng quay lại sau.');
                });
            });
        ...
        init() {
            ...
            $form.on('submit', function (e) {
                ...
                if (tob && !validateTime(tob)) {
                    $('#zdc-error-natal-tob').text('Giờ không hợp lệ');
                    hasError = true;
                } else {
                    $('#zdc-error-natal-tob').text('');
                }
                if (!pob) {
                    $('#zdc-error-natal-pob').text('Vui lòng nhập nơi sinh.');
                    hasError = true;
                } else {
            ...
            $(document).on('click', '#zdc-natal-btn-analyze', function (e) {
                ...
                $.post(ZodiacAjax.api_url + 'natal-analyze', {
                    ...
                    if (resAI.success) {
                        ...
                        $('#zdc-natal-deep-form').find('.zdc-err-analyze').text(resAI.data.message || 'Lỗi không xác định.');
                    ...
                    $btn.removeClass('zdc-loading').removeAttr('disabled');
                    $('.zdc-natal-btn-reset').fadeIn(150);
                    $('#zdc-natal-deep-form').find('.zdc-err-analyze').text('Lỗi kết nối. Vui lòng thử lại sau.');
                    $('#zdc-natal-html').html('').hide();
                    $('html, body').animate({scrollTop: $('#zdc-natal-deep-form').offset().top - 50}, 400);
                });
            });
    ...
    // ── Natal Chart Wheel ──
    const NatalWheel = {
        elementColors: {'Lửa': '#e8543a', 'Đất': '#8B6914', 'Khí': '#4a9e6b', 'Nước': '#3a7fc1'},
        planetSymbols: {
            'Mặt Trời': '☀',
            'Mặt Trăng': '☽',
            'Thủy Tinh': '☿',
        ...
        signSymbols: ['♈', '♉', '♊', '♋', '♌', '♍', '♎', '♏', '♐', '♑', '♒', '♓'],
        signElements: ['Lửa', 'Đất', 'Khí', 'Nước', 'Lửa', 'Đất', 'Khí', 'Nước', 'Lửa', 'Đất', 'Khí', 'Nước'],
        ...
        draw(data) {
            ...
            [['ASC', data.ascendant.degree, '#c0392b', 'Cung Mọc — ' + data.ascendant.sign],
                ['MC', data.midheaven.degree, '#8e44ad', 'Thiên Đỉnh — ' + data.midheaven.sign]].forEach(item => {
