    ...
    public function ftn_groq_generate($prompt) {
        ...
        while (!empty($groq_keys)) {
            ...
            if ($httpCode === 429 || (isset($response['error']['code']) && $response['error']['code'] === 'rate_limit_exceeded')) {
                ...
                $this->sendAdminNotification("Groq API Key Limited", "API Key: $groq_key\nError: 429\nTime: " . current_time("Y-m-d H:i:s"));
                continue;
            }
            ...
            $result = ($finish_reason === 'stop') ? "no information available" : "[Error] Key: $groq_key - Unexpected response.";
            break;
        }
