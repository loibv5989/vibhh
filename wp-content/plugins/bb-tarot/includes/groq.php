...
class TR_Groq {
    ...
    public function ftn_groq_generate($prompt, &$info = null) {

        $helpers   = $this->helpers();
        $groq_keys = $helpers->groqKeysArray();
        $groq_model = $helpers->groqModel();
        $result    = null;
        ...
        while (!empty($groq_keys)) {
            ...
            if ($httpCode === 429 || (isset($response['error']['code']) && $response['error']['code'] === 'rate_limit_exceeded')) {
                ...
                if ($info !== null) {
                    $info['failed_keys'][] = $this->maskKey($groq_key) . ' (429)';
                }
            ...
            if ($httpCode === 498) {
                ...
                if ($info !== null) {
                    $info['failed_keys'][] = $this->maskKey($groq_key) . ' (498)';
                }
                unset($groq_keys[array_search($groq_key, $groq_keys)]);
                continue;
            }
            ...
            if ($httpCode === 503 || (isset($response['error']['code']) && $response['error']['code'] === 503)) {
                ...
                if ($info !== null) {
                    $info['failed_keys'][] = $this->maskKey($groq_key) . ' (503)';
                }
                unset($groq_keys[array_search($groq_key, $groq_keys)]);
                continue;
            }
