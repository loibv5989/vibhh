<?php
/**
 * Template: Cross Spread (5 lá)
 * Layout: Plus Shape (Chữ thập)
 */

if (!defined('ABSPATH')) exit;

function tarot_cross(string $topic, array $cards, string $mode = 'topic', string $question = '', string $spread_key = '5_cards'): string {
    $spreads = require TAROT_PLUGIN_DIR . 'includes/spreads.php';
    $spread_config = $spreads[$spread_key] ?? $spreads['5_cards'];
    ...
    <div id="trt-detail-container" style="display:none">
        ...
        <div class="trt-5cross-area">
            <?php foreach ($cards_data as $i => $data):
                $c = $data['card'];
                $slot_num = $i + 1; // 1 đến 5
                $orient = $c['orientation'];
                $el_class = 'trt-el-' . ($c['element'] ?? 'earth');
                ?>
                <div class="trt-5c-slot trt-5c-slot-<?= $slot_num ?>">
