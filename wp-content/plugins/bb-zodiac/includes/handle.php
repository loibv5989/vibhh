...
class BbZodiac_Handle {
    ...
    public function enqueueAssets(): void {
        ...
        wp_enqueue_script('bb-zodiac', BB_ZODIAC_PLUGIN_URL . 'assets/zodiac.min.js', ['jquery'], BB_ZODIAC_VERSION, true);
    ...
    public function renderShortcode(array $atts, string $content = '', string $tag = 'zodiac_form'): string {
        $template_map = [
            ...
            'zodiac_tinh_cach'    => 'tinh-cach.php',
            'zodiac_tinh_yeu'     => 'tinh-yeu.php',
            'zodiac_tu_vi'        => 'tu-vi.php',
            'zodiac_ban_do_sao'   => 'ban-do-sao.php',
        ];
    ...
    public function handleCalc(WP_REST_Request $request): WP_REST_Response {
        ...
        if ($contactLine !== '') {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Yêu cầu không hợp lệ.']], 200);
        }

        $dob = sanitize_text_field((string)$request->get_param('dob'));
        if (empty($dob)) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Vui lòng nhập ngày sinh.']], 200);
        }
        ...
        if (!empty($hasFuture)) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Tuổi này đợi ra đời rồi tính tiếp.']], 200);
        }
    ...
    public function handleLove(WP_REST_Request $request): WP_REST_Response {
        ...
        if ($nameA === '' || $nameB === '' || $dobA === '' || $dobB === '') {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Vui lòng nhập đủ họ tên và ngày sinh của cả 2 người.']], 200);
        }
    ...
    public function handleLoveAnalyze(WP_REST_Request $request): WP_REST_Response {
        ...
        if (!$this->validate_logged_in()) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Vui lòng đăng nhập để sử dụng tính năng này.']], 200);
        }

        if (get_option('bb_zodiac_allow_ai', '0') !== '1') {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Chức năng này đang tạm ngưng. Vui lòng quay lại sau.']], 200);
        }

        if (!$this->zodiac_quota()) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Đã đạt giới hạn phân tích trong ngày. Vui lòng quay lại vào ngày mai.']], 200);
        }
        ...
        if (empty($calcData) || !is_array($calcData)) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Dữ liệu không hợp lệ.']], 200);
        }
        ...
        if (str_starts_with($rawResponse, '[Error]')) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Không thể kết nối, vui lòng thử lại.']], 200);
        }
    ...
    public function handleNatal(WP_REST_Request $request): WP_REST_Response {
        $contactLine = sanitize_text_field((string)$request->get_param('zdc_cbsp'));
        if ($contactLine !== '') return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Yêu cầu không hợp lệ.']], 200);
        ...
        if (empty($dob) || empty($pob)) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Vui lòng nhập Ngày và Nơi sinh.']], 200);
        }
        ...
        try {
            ...
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Lỗi thiên văn: ' . $e->getMessage()]], 200);
        }
    ...
    public function handleNatalAnalyze(WP_REST_Request $request): WP_REST_Response {
        ...
        if (get_option('bb_zodiac_allow_ai', '0') !== '1') {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Tính năng đang bảo trì.']], 200);
        }
        ...
        $rawResponse = ($providers[$provider] ?? $providers['gemini'])($prompt);
        if (str_starts_with($rawResponse, '[Error]')) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Hệ thống đang bận, vui lòng thử lại sau ít phút.']], 200);
        }

        $parsed = BbZodiac_Calc::parseResponse($rawResponse);
        if (!$parsed['has_zdc_html'] || empty(trim($parsed['tabs']['chi_tiet']))) {
            return new WP_REST_Response(['success' => false, 'data' => ['message' => 'Giải mã thất bại, vui lòng thử lại.']], 200);
        }
