...
class MBTI_Admin {
    ...
    public function __construct(string $plugin_file = '') {
        $this->plugin_file = $plugin_file ?: MBTI_PLUGIN_FILE;

        add_action('admin_menu',            [$this, 'registerSettingsPage']);
        add_action('admin_init',            [$this, 'saveSettings']);
        add_action('admin_enqueue_scripts', [$this, 'enqueueAssets']);
        add_action('wp_ajax_mbti_test_provider', [$this, 'testProvider']);
        add_action('wp_ajax_mbti_create_pages',  [$this, 'createPages']);
        add_filter('plugin_action_links_' . plugin_basename($this->plugin_file), [$this, 'settingsLink']);
    }

    public function enqueueAssets(string $hook): void {
        if (!str_contains($hook, 'mbti-settings')) return;
        wp_enqueue_style('mbti',  MBTI_PLUGIN_URL . 'admin/assets/css/mbti.css', [], MBTI_VERSION);
        wp_enqueue_script('mbti',  MBTI_PLUGIN_URL . 'admin/assets/js/mbti.js',  ['jquery'], MBTI_VERSION, true);
        wp_localize_script('mbti', 'mbtiAdmin', [
            'test_provider_nonce' => wp_create_nonce('mbti_test_nonce'),
            'create_pages_nonce' => wp_create_nonce('mbti_create_pages_nonce'),
        ]);
    }

    public function settingsLink(array $links): array {
        array_unshift($links, '<a href="admin.php?page=mbti-settings">Settings</a>');
        return $links;
    }
    ...
    public function renderSettingsPage(): void {
        $model         = get_option('mbti_ai_model', 'gemini-flash-latest');
        $provider      = get_option('mbti_ai_provider', 'gemini');
        $groq_model    = get_option('mbti_groq_model', 'llama-3.3-70b-versatile');
        $gemini_key    = get_option('mbti_gemini_key', '');
        $groq_key      = get_option('mbti_groq_key', '');
        $mistral_key   = get_option('mbti_mistral_key', '');
        $mistral_model = get_option('mbti_mistral_model', 'mistral-small-latest');
        $allow_ai      = get_option('mbti_allow_ai', '1');
    ...
    public function saveSettings(): void {
        if (
            ...
            !wp_verify_nonce($_POST['_wpnonce'], 'mbti_settings_form') ||
            !current_user_can('manage_options')
        ) return;

        if (isset($_POST['provider'])) {
            update_option('mbti_ai_provider', sanitize_text_field($_POST['provider']));
        }

        if (isset($_POST['model'])) {
            update_option('mbti_ai_model', sanitize_text_field($_POST['model']));
        }

        if (isset($_POST['groq_model'])) {
            update_option('mbti_groq_model', sanitize_text_field($_POST['groq_model']));
        }

        if (isset($_POST['mistral_model'])) {
            update_option('mbti_mistral_model', sanitize_text_field($_POST['mistral_model']));
        }

        update_option('mbti_allow_ai', isset($_POST['allow_ai']) ? '1' : '0');

        if (isset($_POST['analysis_order'])) {
            update_option('mbti_analysis_order', sanitize_text_field($_POST['analysis_order']));
        }

        if (isset($_POST['gemini_key'])) {
            update_option('mbti_gemini_key', sanitize_textarea_field($_POST['gemini_key']));
        }

        if (isset($_POST['groq_key'])) {
            update_option('mbti_groq_key', sanitize_textarea_field($_POST['groq_key']));
        }

        if (isset($_POST['mistral_key'])) {
            update_option('mbti_mistral_key', sanitize_textarea_field($_POST['mistral_key']));
        }
    }

    public function testProvider(): void {
        check_ajax_referer('mbti_test_nonce', 'nonce');
        ...
        try {
            switch ($provider) {
                case 'gemini':
                    $response = MBTI_Gemini::get_instance()->ftn_gemini_generate($test_prompt);
                    break;
                case 'groq':
                    $response = MBTI_Groq::get_instance()->ftn_groq_generate($test_prompt);
                    break;
                case 'mistral':
                    $response = MBTI_Mistral::get_instance()->ftn_mistral_generate($test_prompt);
                    break;
                default:
    ...
    public function createPages(): void {
        if (!check_ajax_referer('mbti_create_pages_nonce', 'nonce', false) || !current_user_can('manage_options')) {
