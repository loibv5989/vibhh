jQuery(function ($) {
    // Provider switching
    $('#mbti_provider_select').on('change', function() {
    ...
    $('#test-provider').on('click', function() {
        var provider = $('#mbti_provider_select').val();
        var $button = $(this);
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'mbti_test_provider',
                provider: provider,
                nonce: mbtiAdmin.test_provider_nonce
            },
    ...
    $('#mbti-create-pages').on('click', function () {
        ...
        $.post(ajaxurl, {
            action: 'mbti_create_pages',
            nonce:  mbtiAdmin.create_pages_nonce,
        }, function (res) {
            if (res.success) {
