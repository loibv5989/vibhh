...
class TuVi_Admin {
    ...
    public function __construct($plugin_file) {
        ...
        add_action('wp_ajax_tuvi_test_provider', [$this, 'test_provider']);
        add_filter('plugin_action_links_' . plugin_basename($this->plugin_file), [$this, 'settings_link']);
    }

    public function settings_link($links) {
        $settings_link = '<a href="admin.php?page=tu-vi">Settings</a>';
    ...
    public function enqueue_admin_assets($hook) {
        if (!str_contains($hook, 'tu-vi')) return;

        wp_enqueue_style(
            'tuvi-admin-css',
            TUVI_PLUGIN_URL . 'admin/assets/css/tuvi-admin.css',
            [],
            TUVI_PLUGIN_VERSION
        );
        ...
        wp_enqueue_script(
            'tuvi-admin-js',
            TUVI_PLUGIN_URL . 'admin/assets/js/tuvi-admin.js',
            ['jquery'],
            TUVI_PLUGIN_VERSION,
            true
        );

        wp_localize_script('tuvi-admin-js', 'tuviAdmin', [
                'ajax_url' => admin_url('admin-ajax.php'),
            'test_provider_nonce' => wp_create_nonce('tuvi_test_nonce'),
        ]);
    }
    ...
    public function render_settings_page() {
        ...
        $allow_ai = get_option('tuvi_allow_ai', '0');
        $provider = get_option('tuvi_ai_provider', 'gemini');

        $gemini_model = get_option('tuvi_gemini_model', 'gemini-flash-latest');
        $groq_model = get_option('tuvi_groq_model', 'llama-3.3-70b-versatile');
        $mistral_model = get_option('tuvi_mistral_model', 'mistral-small-latest');

        $gemini_key = get_option('tuvi_gemini_key', '');
        $groq_key = get_option('tuvi_groq_key', '');
        $mistral_key = get_option('tuvi_mistral_key', '');

        $gatekeeper_order = get_option('tuvi_gatekeeper_order', 'groq,mistral,gemini');
        $analysis_order = get_option('tuvi_analysis_order', 'gemini,mistral,groq');

        ?>
        <div class="wrap">
            <h1>Cấu hình | Tử Vi</h1>
            <form method="post">
                <?php wp_nonce_field('tuvi_settings_form'); ?>
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            <select name="provider" id="tuvi_provider_select">
                            ...
                            <button type="button" id="tuvi-test-provider" class="button" style="margin-left: 10px;">Test Connection</button>
                            <div id="tuvi-test-results" style="margin-top: 10px;"></div>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row">AI Model</th>
                        <td>
                            <div id="tuvi-gemini-models" style="<?php echo $provider === 'gemini' ? '' : 'display:none'; ?>">
                            ...
                            <div id="tuvi-groq-models" style="<?php echo $provider === 'groq' ? '' : 'display:none'; ?>">
                            ...
                            <div id="tuvi-mistral-models" style="<?php echo $provider === 'mistral' ? '' : 'display:none'; ?>">
                ...
                <div id="tuvi-gemini-settings" style="<?php echo $provider === 'gemini' ? '' : 'display:none'; ?>">
                    <table class="form-table">
                        <tr>
                ...
                <div id="tuvi-groq-settings" style="<?php echo $provider === 'groq' ? '' : 'display:none'; ?>">
                    <table class="form-table">
                        <tr>
                ...
                <div id="tuvi-mistral-settings" style="<?php echo $provider === 'mistral' ? '' : 'display:none'; ?>">
                    <table class="form-table">
                        <tr>
    ...
    public function test_provider() {
        check_ajax_referer('tuvi_test_nonce', 'nonce');

        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => 'Permission denied']);
        }

        if (get_option('tuvi_allow_ai', '0') !== '1') {
        ...
        try {
            switch ($provider) {
                case 'gemini':
                    $response = Tuvi_Gemini::get_instance()->ftn_gemini_generate($test_prompt);
                    break;
                case 'groq':
                    $response = Tuvi_Groq::get_instance()->ftn_groq_generate($test_prompt);
                    break;
                case 'mistral':
                    $response = Tuvi_Mistral::get_instance()->ftn_mistral_generate($test_prompt);
                    break;
                default:
    ...
    public function save_settings() {
        ...
        if (!wp_verify_nonce($_POST['_wpnonce'], 'tuvi_settings_form')) {
            return;
        }

        $allow_ai = isset($_POST['allow_ai']) ? '1' : '0';
        update_option('tuvi_allow_ai', $allow_ai);
        ...
        update_option('tuvi_gemini_model', $gemini_model);
        update_option('tuvi_groq_model', $groq_model);
        update_option('tuvi_mistral_model', $mistral_model);
        ...
        update_option('tuvi_gemini_key', $gemini_key);
        update_option('tuvi_groq_key', $groq_key);
        update_option('tuvi_mistral_key', $mistral_key);
    }
}
